...
> **一期已落地**（叶子 AABB × 六平面）。二期语义树剪枝、三期 BVH 视锥还没做。渲染基线见 [渲染管线](RENDERING.md)；视锥怎么变成 NDC 立方体见 [视锥、NDC 与屏幕](NDC.md)；语义树 / `world_bounds` / 展平见 [语义树](SCENE-GRAPH.md)。
...
- **不做渲染场景图。** 语义树留在 `Scene` / Document；渲染仍只拿平铺的 `SceneDrawItem`。BVH 是语义树之外的加速结构，和 [语义树](SCENE-GRAPH.md) 里「空间索引不是把语义树搬进渲染」一致。

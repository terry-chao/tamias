...
> **现状：还没做。** 本文是落地顺序，不是当前代码行为。渲染基线见 [渲染管线](RENDERING.md)；视锥怎么变成 NDC 立方体见 [视锥、NDC 与屏幕](NDC.md)；语义树 / `world_bounds` / 展平见 [场景图](SCENE-GRAPH.md)。
...
| `perspective * look_at`（CPU 按 OpenGL 习惯，Y-up） | 视锥平面 |
| 拾取 BVH（世界盒，只给点选用） | 绘制前跳过屏外叶子 |
| `draw_channel` 对每个 item 一次 draw | 合批 / instancing |

当前展平是直通的：`Document::render_items()` 扫一遍语义树，有网格就塞进 `SceneDrawItem`，不管在不在画面里。视口把整份清单交给 `RenderThread`。
...
测试（每个平面一次）：取 AABB 上沿法线**最靠外**的那个角（p-vertex）。该角已在外侧 ⇒ 整盒在外 ⇒ 剔除。六个平面都没把盒子完全踢掉 ⇒ 保留。盒子无效（`!valid()`）⇒ 保留，避免漏画。
...
场景扁平时（墙全挂在根上）已经能砍掉绝大部分 draw。实现小、可单测。
...
| [math.h](https://github.com/terry-chao/tamias/blob/main/src/engine/math/math.h) `Aabb` / `perspective` / `look_at` | 盒子与相机矩阵 |

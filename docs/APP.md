...
- **分组可以放到第二排**：拖动期间 Ribbon 下方会亮出一条空的排当落点，松手就停在那一排；
  第二排没有组时自动收起，页高 = 可见排数 × 单排高度（92 / 仅图标 54）。
- 浮着的位置 / 哪几组浮着，记在设置里（`ui/ribbon_floating_groups`），下次开还在那儿。

实现都在 `shell/ribbon_*`：`ribbon_bar`（页签、形态、拖放、浮窗管理）、`ribbon_page`
（两排布局、分组的停靠 / 摘出、落点线）、`ribbon_group`（按钮与抓手，拖拽源）、`ribbon_float_window`

...
| 形态 | 长什么样 | 何时用 |
|---|---|---|
| 图标 + 文字（默认） | 每个工具一张 24px 图标 + 名字，分组下面还有组名 | 教学、不熟快捷键时 |
| 仅图标（`ui/ribbon_style=icons`） | 只有图标，鼠标悬浮出名字（FreeCAD 那种），整条带子也更矮 | 熟练用户，省纵向空间 |

**个别按钮可以只留图标**：图标本身已经说清楚它是什么的（现在只有「设置」这么做——新建 /
打开旁边全是带字的按钮，独独少一行名字会像缺了一块），在 `main_window` 里给那个 action
打一个 `ribbonIconOnly` 属性就不画名字了（见 `RibbonGroup::apply_style_to`）——名字仍在
悬浮提示与菜单里，组也跟着窄一点。这种按钮夹在一排带字的中间时，外框会撑到和它们一样高
（`ribbonLabelLess`），免得两个图标看着一大一小。

...
| [document_viewport.cpp](https://github.com/terry-chao/tamias/blob/main/src/app/viewport/canvas/document_viewport.cpp) / [viewer_host.cpp](https://github.com/terry-chao/tamias/blob/main/src/wasm/viewer_host.cpp) | 消费侧组装帧 | 每帧提交 `scene_generation` + `dirty_since(上次游标)`；视口隐藏过滤（floor/类别/isolate）收集成 `hidden_node_ids` |
...
| **空间索引 + 实例化** | BIM 规模（几万构件）的剔除与合批 | 视锥一期已落地，见 [视锥剔除](FRUSTUM-CULLING.md)；G1a intern 已落地，GPU instance 见 [合批 / Instancing](INSTANCING.md)。加速结构不是场景图。亿级三角见 [超大规模三角](MASSIVE-GEOMETRY.md) |

# 场景图深度分析：OCCT 的实现 vs Tamias 的语义树 + 展平渲染

> 面向想搞清一件事的人：**「场景图」到底是什么、OCCT 里的场景图实现到哪一步、Tamias 现在用的那棵「树」和它是什么关系、以及作为 BIM 应用这条路为什么是对的。** 本文是若干次讨论的沉淀，结论都用 Tamias 的实际代码逐一验证过。三角网如何画到像素见 [渲染管线](RENDERING.md)。

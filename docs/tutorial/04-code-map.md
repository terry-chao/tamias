...
│   ├── base/     应用基础设施：设置、主题、翻译、最近文件、后端登记
│   ├── shell/    窗口骨架：主窗口、欢迎页、Ribbon、通用对话框
│   ├── viewport/ 三维视口：canvas/ 画布本体、overlay/ 浮层（ViewCube、框选）、panel/ 工具列、replay/ 回放
│   ├── bim/      构件的界面：属性 / 绘制规格 / 显隐 / 楼层 / 轴网（各自一个子目录）
...
│   ├── core/      实体基类、夹点、识别色
│   ├── family/    族实体：FamilyEntity 基类 + 按「在宿主关系里的角色」分的子目录
│   │   ├── host/    宿主实体：能独立存在、能承接附属件与开洞的构件
│   │   │   ├── architectural/ 建筑：墙、幕墙
│   │   │   └── structural/    结构：梁、柱、板、结构墙、基础
│   │   └── attached/ 附属实体：只能挂在宿主上
│   │       └── opening/       扣减实体：门、窗（在宿主上开洞）
│   ├── sketch/    草图曲线：直线、折线、圆、圆弧、贝塞尔、矩形、B 样条、NURBS
│   └── primitive/ 基础体：盒子、圆柱
├── bim/         BIM 业务层：楼层、宿主、关联关系、IFC 空间结构
├── engine/
│   ├── base/    日志、结果类型、路径/平台工具（基础设施）
│   ├── math/    向量/矩阵、相机、网格

...
**现状**：第一步「索引 + 几何指纹」已落地——追加圆角 / 倒角时记下那条边的归一化中点、方向、相对长度和相邻面法线，重算时先核对指纹、对不上再全量匹配，匹配不到就报错而不是随便倒一条。这已经是上表「几何法」的匹配思路，但只覆盖圆角 / 倒角引用的单条边，还没有特征级的持久命名传播。详见 [特征树求值器](FEATURE-TREE-EVALUATOR.md) §9。
...
| Topological Naming | P3（先索引法，再几何法） | 索引 + 几何指纹已落地；几何法 / 持久命名未做 |

...
2. **拓扑命名（Topological Naming，最难、最著名的坑）。** 「给这条边倒圆角」引用的是「当前 BRep 里的边 #N」，但改了上游参数后 BRep 重算、边的编号全变，圆角就倒错边了。解法从简单到复杂：**索引法**（按编号，简单但脆）→ **几何法**（按边的位置/方向/相邻面匹配，稳但复杂）。FreeCAD 被此问题折磨多年。**现状：第一步（索引 + 几何指纹）已落地**——创建圆角/倒角时记下那条边的几何长相，重算时先核对、再全量匹配、匹配不到就报错；几何法 / 持久命名还没做。详见 [特征树求值器](FEATURE-TREE-EVALUATOR.md) §9。
3. **命令化 undo。** 现在 [history.h](https://github.com/terry-chao/tamias/blob/main/src/command/core/history.h) 是全量快照，几何编辑多了会重。目标：每条操作（改参数/加特征）是一条可逆命令。
...
| **P3** | 拓扑命名：索引 + 几何指纹（已落地）→ 几何法 / 持久命名 | 改上游参数不炸下游 |
...
| **`.tdoc` → LevelDB** | 内部工作文档从整文件 `binary_archive` 改成 LevelDB；支撑增量保存、大模型局部加载。**现在不做**，方向已定 |

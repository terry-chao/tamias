...
按楼层显隐不写文档：楼层面板勾选/取消只改视口的隐藏集合。**归属优先看 Location 的楼层**（BIM 语义）：在 1 楼画的东西就属于 1 楼——1 楼的顶板压在 2 楼标高上，也仍然算 1 楼（`viewport_floor_allows`）。没有楼层归属的（未归属构件、导入网格）退回几何楼层带（`viewport_floor.h`）：本层到上一层，顶层用 `elevation + height`；跨层构件碰到任意一个可见带就还看得见。
...
「楼层管理」页把楼层当**视图清单**用：第一行是全局三维（默认打开的就是它），下面一层一行，双击某层 = 打开该层视图——只留这一层（同一套楼层带过滤）、把它设为当前楼层、切到平面（2D）并框到这一层。视图状态只活在视口里（`floor_view_`），不写文档；切回三维或「全部显示」就回到全局三维。
...
- [wall_entity.cpp](https://github.com/terry-chao/tamias/blob/main/src/entity/family/architectural/wall_entity.cpp) 等 —— 构件几何配方，不是业务层

...
**现状：** 没有楼层系统，也没有轴网。墙梁板柱已经能画，但入树时 `parent = 0`，等于未归属。本文是架构落点，不是已实现代码。

代码将来放 [`src/bim/`](https://github.com/terry-chao/tamias/tree/main/src/bim)（尚未建）。实体几何仍在 [`src/entity/`](https://github.com/terry-chao/tamias/tree/main/src/entity)，命令仍在 [`src/command/`](https://github.com/terry-chao/tamias/tree/main/src/command)。
...
| 方向 | 本层做什么 | 不做什么 |
|---|---|---|
| 对上（command / app） | `place_wall` / `create_storey` / `set_active_storey` | 不碰 Qt、不自己 push undo |
| 对下（Document / Scene） | `add_entity`、建分组节点、`set_parent` | 不改 `recompute_world`、不解释网格 |
| 对造型 | 需要几何时调 `Entity::createGeom` | 不写特征树、不调 OCCT |
| 对渲染 | 轴网/标高需要 overlay 时只出数据 | 不提交 GPU、不改 draw list |

`SceneNode.parent` 仍是**唯一的树真相**。本层决定「该写成谁」，语义树只记账。渲染展平后仍然不知道「这是 2 楼」——按楼层显隐由本层查出一串 id，再变成可见性标志交给渲染。详见 [语义树](SCENE-GRAPH.md)。
...
| 当前标高 | 会话/文档上的「正在画哪一层」 | 不是 Scene 字段；本层持有 `active_storey_id` |
| 墙梁板柱门窗 | 放置、归属楼层、后改宿主 | 叶子节点；`parent` 指向楼层（或门窗指向宿主墙） |
| 轴网 | 定位参考，不是实体构件 | 数据在本层；显示走 overlay，不进普通网格叶子 |
| 宿主 | 窗属于墙、墙属于楼层 | 显式关系；不靠 Z 坐标反推 |
...
**现在：** 没有楼层对象。`add_entity` 不写 `parent`，墙挂在根上。这是合法的「未归属」，不是 bug。
...
```
CreateWallCommand
  → BimModel::place_wall(start, end, thickness, height)
      → WallEntity + createGeom
      → Document::add_entity(...)          // 仍通用
      → Scene::set_parent(wall, active_storey_node)  // 仅当有当前标高
```
...
```
src/bim/storey.h       // Storey：id、名称、标高、对应的 SceneNode id
src/bim/grid.h         // 轴网
src/bim/bim_model.h    // 楼层表、轴网、当前标高；place_wall / assign_host
```
...
本层**先不做**：墙连接自动裁剪、核心层构造、房间边界生成、全专业 MEP、Revit 式约束系统。那些会把这一层做成第二个 Revit。
...
- [create_wall_command.cpp](https://github.com/terry-chao/tamias/blob/main/src/command/create_wall_command.cpp) —— 今日直写 Document；落地后改调 `BimModel`
- [scene.h](https://github.com/terry-chao/tamias/blob/main/src/engine/document/scene.h) —— `parent`；不出现 Storey / Grid 类型

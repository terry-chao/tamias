...
按楼层显隐不写文档：楼层面板勾选/取消只改视口的隐藏集合。**归属优先看 Location 的楼层**（BIM 语义）：在 1 楼画的东西就属于 1 楼——1 楼的顶板压在 2 楼标高上，也仍然算 1 楼（`viewport_floor_allows`）。没有楼层归属的（未归属构件、导入网格、还没有楼层表的老模型）退回几何楼层带（`viewport_floor.h`）：本层到上一层，顶层用 `elevation + height`；跨层构件碰到任意一个可见带就还看得见。
